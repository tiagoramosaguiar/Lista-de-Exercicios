        ...
        private void ButtonCalcular_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (razao == 0)
                {
                    ...
                    MessageBox.Show("Erro! A razão deve ser diferente de zero.", "Progressão geométrica", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                MessageBox.Show(mensagem, "Progressão geométrica");
                Funcoes.LimparControles(this);
                TextElementoInicial.Focus();
            }
            catch (FormatException)
            {
                MessageBox.Show("Erro! Preencha todos os campos.", "Progressão geométrica", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch (OverflowException)
            {
                MessageBox.Show("Erro! Valor inserido muito grande.", "Progressão geométrica", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}